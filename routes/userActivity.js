...
router
  ...
  .get(async (req, res) => 
  {
    ...
    try
    {
        activity = await userActivity.getUserActivity(req.session.user.stevensEmail);
    } catch (e)
    {
        return res.status(400).render('error', {title:"User Activity Error", error:e});
    }
    return res.render('userActivity', {title:"User Page", css: "userActivity", js: "userActivity", user: req.session.user, activity: activity});
  });

router.route('/setting')
  .get(async (req, res) =>
  {
    return res.render('userSetting', {title:"Settings", css: "userActivity", js: "userActivity", user: req.session.user});
  })
  .post(async (req, res) =>
  {
    ...
    {
      return res.status(400).render('userSetting', {title:"Settings", error:e, css: "userActivity", js: "userActivity", user: req.session.user});
    }
    ...
    try
    {
      ...
      {
        output = await users.updateFirstName(req.session.user.stevensEmail, firstName);
      }
      if (req.session.user.lastName != lastName)
      {
        output = await users.updateLastName(req.session.user.stevensEmail, lastName);
      }
      if (req.session.user.academicStatus != academicStatus)
      {
        output = await users.updateAcademicStatus(req.session.user.stevensEmail, academicStatus);
      }
      // update the session
